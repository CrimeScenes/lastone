...
-- Enhanced prediction calculation with velocity smoothing
local function getPredictedPosition(character)
    ...
    if primaryPart then
        ...
        -- Smoothing the velocity for better prediction
        local smoothedVelocity = (velocity + primaryPart.Velocity) / 2

        return primaryPart.Position + (smoothedVelocity * predictionMultiplier * timeToPredict)
    end
    return nil
end
...
local function handleShootingMode()
    if getgenv().Forbidden.TriggerBot.Settings.Mode == "toggle" then
        ...
        RunService:BindToRenderStep("TriggerBotHold", Enum.RenderPriority.Input.Value, function()
            TriggerBotAction()  -- Call TriggerBotAction directly
        end)
    end
end
